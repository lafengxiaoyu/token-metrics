...
describe('cross-platform data paths', () => {
  it('honors an explicit Copilot home on every platform', () => {
    ...
    expect(resolveCopilotHome({}, '/home/user')).toBe('/home/user/.copilot')
  })

  it('uses XDG directories on Linux', () => {
    const env = { XDG_CACHE_HOME: '/var/cache/user', XDG_CONFIG_HOME: '/var/config/user' }
    expect(resolveTokenLensCacheDir(env, 'linux', '/home/user')).toBe('/var/cache/user/tokenlens')
    expect(resolveTokenLensConfigDir(env, 'linux', '/home/user')).toBe('/var/config/user/tokenlens')
  })
  ...
  it('uses native application directories on Windows', () => {
    ...
    expect(resolveTokenLensCacheDir(env, 'win32', 'C:\\Users\\dev')).toContain('AppData\\Local/tokenlens/cache')
    expect(resolveTokenLensConfigDir(env, 'win32', 'C:\\Users\\dev')).toContain('AppData\\Roaming/tokenlens')
  })
