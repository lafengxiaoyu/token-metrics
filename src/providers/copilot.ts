...
function parseLegacyEvents(content: string, sessionId: string, seenKeys: Set<string>): ParsedProviderCall[] {
  ...
  for (const line of lines) {
    ...
    if (event.type === 'user.message') {
      ...
      pendingUserMessage = data.content ?? ''
      continue
    }

    if (event.type === 'assistant.message') {
      const data = event.data as { messageId: string; outputTokens: number; toolRequests?: LegacyToolRequest[] }
      const { messageId, outputTokens, toolRequests = [] } = data
      ...
      const tools = toolRequests
        .map((t: LegacyToolRequest) => t.name ?? '')
        .filter(Boolean)
        .map((n: string) => toolNameMap[n] ?? n)
